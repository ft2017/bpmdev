...
function BTfyyy_onclick(){
  ...
  var SQLClaused = new Array(sql);
  var SQLLabel document.write('<script type="text/javascript" src="../../CustomJsLib/EFGPShareMethod.js"></script>');//for 开窗
...
var fun= new add2;
  ...
  if (gridData.length > 0){       
    for (i = 0; i < gridData.length; i++){
      if(gridData[i][8] != ""){
        tSellingPriceTemp = tSellingPriceTemp + eval(gridData[i][8]);
      }     
    }       
    document.getElementById("TBbxzje").value = eval(tSellingPriceTemp ); 
  }
...
// function daxie(){
//   String num = document.getElementById("TBbxzje").value;
